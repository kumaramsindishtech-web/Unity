...
Shader "Industrial Reactor/Water"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Varyings { float4 positionCS : SV_POSITION; float2 uv : TEXCOORD0; float3 positionWS : TEXCOORD1; float3 normalWS : TEXCOORD2; float3 viewDirWS : TEXCOORD3; float fogFactor : TEXCOORD4; };

            TEXTURE2D(_BaseMap); SAMPLER(sampler_BaseMap);

            CBUFFER_START(UnityPerMaterial)
                float4 _BaseMap_ST; float4 _BaseColor; float _WaterLevel; float _Transparency; float _FresnelPower;
                float _SwirlSpeed; float _SwirlStrength; float4 _SwirlCenter; float _Turbulence; float _TurbulenceScale; float _TurbulenceSpeed;
                float4 _EmissionColor; float _EmissionIntensity; float _EmissionPulseSpeed; float _Smoothness; float _BumpScale;
            CBUFFER_END
            ...
            {
                float swirlAngle = _SwirlSpeed * _Time.y * 0.0174533;
                float2 swirlUV = SwirlUV(input.uv, _SwirlCenter.xy, swirlAngle, _SwirlStrength);
                float2 turbulenceOffset = float2(noise(swirlUV * _TurbulenceScale + _Time.y * _TurbulenceSpeed), noise(swirlUV * _TurbulenceScale + _Time.y * _TurbulenceSpeed + 100)) * _Turbulence * 0.1;
